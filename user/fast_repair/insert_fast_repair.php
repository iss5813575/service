...
if (isset($_POST['txt_room'])) {

    $id_repair = countId();
    $field = array();
    $field["id_repair"] = $id_repair;
    $field["repair_room"] = $_POST['txt_room'];
    $field["repair_detail"] = $_POST['txt_detail'];
    $field["date"] = date("Y-m-d H:i:s");
    $field["create_by"] = $_POST["txt_user_id"];
    $field["create_by_name"] = $_POST["txt_user_name"];
    $res = insert_sql("db_fast_repair", $field);
    if ($res) {
...
{
    global $connection;
    $res = mysqli_query($connection, "SELECT id_repair FROM db_fast_repair ORDER BY id_repair DESC  LIMIT 1");
    if (mysqli_num_rows($res) == 0) {
        return 'REP' . date("Y") . '0001';
    } else {
        $rw = mysqli_fetch_array($res);
        if (substr($rw['id_repair'], 3, -4) == date("Y")) {
            $no = (substr($rw['id_repair'], 7) * 1) + 1;
            $num = str_pad($no, 4, "0", STR_PAD_LEFT);
            return 'REP' . date("Y") . $num;
        } else {
            return 'REP' . date("Y") . '0001';
        }
    }
